...
import { classifyIntent } from '../ai/sentiment';
...
export const inboxRouter = router({
  ...
  /**
   * inbox.classifyIntent
   * Uses Claude to classify a single inbox item's intent
   * (question, complaint, praise, opportunity, spam).
   */
  classifyIntent: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const result = await classifyIntent(item.content || '');

      await ctx.prisma.inboxItem.update({
        where: { id: input.id },
        data: {
          internalNotes: JSON.stringify({
            ...(item.internalNotes ? JSON.parse(item.internalNotes) : {}),
  ...
  /**
   * inbox.classifyAll
   * Batch-classifies all unread, non-archived inbox items.
   */
  classifyAll: protectedProcedure
    .mutation(async ({ ctx }) => {
      ...
      const results = { classified: 0, failed: 0 };
      ...
      for (const item of items) {
        try {
          ...
          console.error(`Failed to classify inbox item ${item.id}:`, err.message);
          results.failed++;
        }
      }
