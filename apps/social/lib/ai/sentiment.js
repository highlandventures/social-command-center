...
Analyze sentiment of each text. Consider sarcasm and context. Return one object per text in order.
Schema: [{"index":number,"sentiment":"POSITIVE|NEGATIVE|NEUTRAL","confidence":number,"reasoning":"string"}]`;
...
export async function classifyIntent(text) {
  const systemPrompt = `${AI_PREAMBLE}
Classify intent as exactly one of: question, complaint, praise, opportunity, spam.
Schema: {"intent":"question|complaint|praise|opportunity|spam","confidence":number,"reasoning":"string"}`;
...
export async function extractThemes(texts) {
  const systemPrompt = `${AI_PREAMBLE}
Identify 3-8 recurring themes from social media posts, ordered by frequency.
Schema: {"themes":[{"name":"string","frequency":number,"sentiment":"POSITIVE|NEGATIVE|NEUTRAL|MIXED","representativeTexts":["string"]}]}`;
