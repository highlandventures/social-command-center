...
const HomePage = ({ onNavigate }) => (
  <>
    ...
    <Section><SectionTitle sub="Not just a stablecoin. A principal-protected security that behaves like one.">What makes YLDS different</SectionTitle>
      <Grid cols={4}><Card title="Earn ~4% by just holding it.">YLDS accrues yield every single day.* USDC and USDT pay you zero. You don't need to stake, lock, or do anything. Just hold it.</Card><Card title="Send it to anyone, anytime.">Transfer peer-to-peer on supported chains. No intermediaries. No waiting. 24/7/365.</Card><Card title="Settle at $1.00. Always.">Redeem to USD or USDC during banking hours. $1.00 NAV, KPMG-audited. No gates, no surprises.</Card><Card title="Built for what's coming">Registered under the Investment Company Act. GENIUS Exempt Issuer backed by GENIUS Compliant Collateral.</Card></Grid>
    </Section>

    <Section bg={S.purple} style={{ padding: '80px 48px' }}>
      <Grid cols={5} style={{ marginBottom: 48 }}><MetricCard val="$609M+" label="Ecosystem AUM" light /><MetricCard val="~4.0%" label="Current Yield (Variable)" light /><MetricCard val="$5.8M" label="Interest Payable" light /><MetricCard val="3" label="Chains Live" light /><MetricCard val="24/7" label="Settlement" light /></Grid>
    ...
    <Section bg={S.lightBg} style={{ padding: '96px 48px' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        <FAQItem q="How does YLDS relate to the GENIUS Act?" a="The GENIUS Act (signed into law July 2025) prohibits payment stablecoin issuers from paying yield to holders. YLDS is a registered security — not a payment stablecoin — making Figure Certificate Corporation a GENIUS Exempt Issuer, backed by GENIUS Compliant Collateral. The same logic applies to the pending CLARITY Act." />
        ...
        <FAQItem q="How do I buy YLDS?" a="You can get YLDS directly through ylds.com — you'll be prompted to onboard or connect an authorized wallet. There are no limits on purchases or redemptions. For large-volume allocations or institutional onboarding, contact our team." />
  ...
  <>
    ...
    <Section bg={S.dark} style={{ padding: '80px 48px' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
        ...
        <Grid cols={3}>{[{date:'2024',t:'YLDS registered',d:'Filed and registered under the Investment Company Act as a fixed-income security.'},{date:'July 2025',t:'GENIUS Act signed',d:'Bans payment stablecoin issuers from paying yield to holders. YLDS is a GENIUS Exempt Issuer.'},{date:'Pending',t:'CLARITY Act',d:'Extends yield restrictions to platforms. YLDS structured to remain exempt.'}].map((e,i)=><div key={i} style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:16,padding:24,textAlign:'center'}}><div style={{fontSize:12,fontWeight:700,textTransform:'uppercase',letterSpacing:1,color:S.accent,marginBottom:8}}>{e.date}</div><h3 style={{fontSize:16,fontWeight:700,marginBottom:6,color:'white'}}>{e.t}</h3><p style={{fontSize:13,color:'rgba(255,255,255,0.5)',lineHeight:1.5}}>{e.d}</p></div>)}</Grid>
    ...
    <Section bg={S.purple} style={{ padding: '80px 48px' }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <CompTable headers={['','YLDS','USDC','USDT','sDAI','BUIDL']} rows={[['Daily yield to holders','~4%*','0%','0%','Variable','~4–5%'],['Registered security','Yes','No','No','No','Yes'],['GENIUS Act status','Exempt Issuer','Restricted','Restricted','Restricted','N/A'],['P2P transferable','Yes','Yes','Yes','Yes','Restricted'],['DeFi composable','wYLDS','Native','Native','Native','No'],['KPMG-audited reserves','Yes','No (attestation)','No','No','Yes'],['Dollar peg','$1.00 NAV','$1.00','$1.00','Variable','$1.00']]} />
      </div>
    </Section>

    <Section><SectionTitle sub="Real treasuries putting idle reserves to work.">How crypto organizations use YLDS</SectionTitle>
      <Grid cols={3}>
        <CaseStudy name="Ondo" type="Treasury Allocation" title="$25M+ YLDS position" desc="Allocated treasury reserves into YLDS as a registered, yield-accruing alternative to idle stablecoin holdings." stat="$25M+" statLabel="YLDS purchased" />
    ...
    <div style={{ background: S.lightBg, padding: '64px 48px' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 16, flexWrap: 'wrap' }}>{['Ondo','Jump Trading','Saluda Grade','Galaxy Digital','Stellar'].map((n,i)=><PlaceholderLogo key={i}>{n}</PlaceholderLogo>)}</div>
      </div>
    </div>
  ...
  <>
    ...
    <Section>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'center' }}>
        <div><h2 style={{ fontSize: 36, fontWeight: 500, marginBottom: 8 }}>Onchain cash sweep</h2><p style={{ color: S.gray, fontSize: 16, lineHeight: 1.7, marginBottom: 16 }}>Cash sweep is a standard treasury operation — excess cash moves into a higher-yielding instrument, then sweeps back when needed. YLDS brings this to digital assets with a lower barrier to entry than traditional demand deposit networks.</p><p style={{ color: S.gray, fontSize: 16, lineHeight: 1.7 }}>Set a reserve threshold. Excess auto-mints to YLDS. When outflows require it, YLDS redeems back to USD. No manual intervention.</p></div>
    ...
    <Section bg={S.purple} style={{ padding: '80px 48px' }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <CompTable headers={['','YLDS','Money Market Fund','Stablecoins']} rows={[['Yield to holders','~4%*','~4–5%','0%'],['On-chain transferable','P2P','No','Yes'],['24/7 settlement','Yes','T+1','Yes'],['Registered security','Yes','Yes','No'],['GENIUS Act status','Exempt Issuer','N/A','Restricted'],['Cross-border settlement','<$0.01','Wire fees','Variable'],['Audited reserves','KPMG','Varies','Attestation']]} />
      </div>
    </Section>
    ...
    <div style={{ background: S.lightBg, padding: '64px 48px' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 16, flexWrap: 'wrap' }}>
          {['Ondo','Jump Trading','Saluda Grade','Galaxy Digital','Stellar','Toku'].map((n,i)=><PlaceholderLogo key={i}>{n}</PlaceholderLogo>)}
        </div>
      </div>
    </div>
