...
export async function generateWeeklyReport(data) {
  const systemPrompt = `You are a senior social media strategist generating a weekly performance report for a social media management platform.
Produce a structured, actionable report based on the provided data.
Always respond with valid JSON matching this exact schema:
{
  "executiveSummary": "string - 2-3 sentence high-level overview of the week",
  "keyMetrics": {
    "totalImpressions": number,
    "totalEngagements": number,
    "avgEngagementRate": number,
    "followerGrowth": number,
    "topPlatform": "X" | "REDDIT",
    "weekOverWeekChange": "string describing trend"
  },
  "topContent": [
    { "postId": "string", "title": "string - brief description", "impressions": number, "engagementRate": number, "whyItWorked": "string" }
  ],
  "audienceInsights": [
    { "insight": "string", "evidence": "string", "actionability": "HIGH|MEDIUM|LOW" }
  ],
  "recommendations": [
    { "recommendation": "string", "priority": "HIGH|MEDIUM|LOW", "expectedImpact": "string", "effort": "LOW|MEDIUM|HIGH" }
  ],
  "outlook": "string - forward-looking assessment for next week"
}`;
...
export async function generateCompetitiveAnalysis(ourData, competitorData) {
  const systemPrompt = `You are a competitive intelligence analyst for a social media management platform.
Generate a competitive analysis report comparing our social media performance against competitors.
Always respond with valid JSON matching this exact schema:
{
  "overview": "string - 2-3 sentence summary of competitive landscape",
  "strengths": [
    { "area": "string", "detail": "string", "evidence": "string" }
  ],
  "threats": [
    { "competitor": "string", "threat": "string", "severity": "HIGH|MEDIUM|LOW", "detail": "string" }
  ],
  "opportunities": [
    { "opportunity": "string", "rationale": "string", "effort": "LOW|MEDIUM|HIGH", "potentialImpact": "string" }
  ],
  "shareOfVoice": {
    "ourShare": number,
...
export async function generateKOLScorecard(kol, activations, metrics) {
  const systemPrompt = `You are a KOL (Key Opinion Leader) performance evaluator for a social media management platform.
Generate a comprehensive scorecard for the provided KOL based on their activations and metrics.
Always respond with valid JSON matching this exact schema:
{
  "score": "A" | "B" | "C" | "D" | "F",
  "rationale": "string - detailed explanation of the assigned grade",
  "highlights": [
    { "highlight": "string", "impact": "string" }
  ],
  "concerns": [
    { "concern": "string", "severity": "HIGH|MEDIUM|LOW", "suggestion": "string" }
  ],
  "recommendations": [
    { "recommendation": "string", "priority": "HIGH|MEDIUM|LOW", "expectedOutcome": "string" }
  ],
  "costEfficiency": {
    "costPerEngagement": number or null,
    "costPerImpression": number or null,
...
export async function generateListeningSummary(hits, topics) {
  const systemPrompt = `You are a social listening analyst for a social media management platform.
Generate a weekly landscape summary from social listening data.
Always respond with valid JSON matching this exact schema:
{
  "topThemes": [
    { "theme": "string", "volume": number, "sentiment": "POSITIVE|NEGATIVE|NEUTRAL|MIXED", "trend": "RISING|STABLE|DECLINING" }
  ],
  "sentimentOverview": {
    "positivePct": number,
    "neutralPct": number,
  ...
  "emergingTopics": [
    { "topic": "string", "signals": "string", "potentialImpact": "HIGH|MEDIUM|LOW", "recommendedAction": "string" }
  ],
  "actionableInsights": [
    { "insight": "string", "urgency": "HIGH|MEDIUM|LOW", "suggestedAction": "string", "relatedHits": number }
  ],
  "competitorActivity": [
    { "competitor": "string", "activity": "string", "sentiment": "POSITIVE|NEGATIVE|NEUTRAL", "implication": "string" }
  ]
}`;
