...
export const postsRouter = router({
  ...
  publish: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Reddit deferred — no API key
      if (post.platform === 'REDDIT') {
        throw new TRPCError({
          code: 'BAD_REQUEST',
          message: 'Reddit publishing is not yet enabled. Reddit API integration coming soon.',
        });
      }

      // Get a valid (non-expired) access token for this account
      ...
      try {
        if (post.contentType === 'THREAD' && post.threadPosts.length > 0) {
          // Publish thread: array of tweet texts, chained as replies
          ...
          // Update each thread child with its platformPostId
        ...
        throw new TRPCError({
          ...
          message: `Failed to publish to X: ${err.message}`,
        });
      }
