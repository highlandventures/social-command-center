...
async function main() {
  ...
  // ------------------------------------------------------------------
  // 6. Sample listening topics with queries
  // ------------------------------------------------------------------
  const topic1 = await prisma.listeningTopic.create({
    data: {
      name: 'Highland Ventures Brand Mentions',
      description:
        'Track mentions of Highland Ventures across X and Reddit',
      createdById: admin.id,
      active: true,
      pollingTier: 'HOT',
      queries: {
        create: [
          {
            platform: 'X',
            queryString: '"Highland Ventures" OR @highland_vc OR @highland_official',
            negativeKeywords: ['highlands', 'hiking'],
            active: true,
          },
          {
            platform: 'REDDIT',
            queryString: '"Highland Ventures" OR "highland vc"',
  ...
  console.log(`  Created listening topic: ${topic1.name}`);

  const topic2 = await prisma.listeningTopic.create({
    data: {
      name: 'Web3 VC Landscape',
      description:
        'Monitor conversations about venture capital in the Web3 and crypto space',
      createdById: admin.id,
      active: true,
      pollingTier: 'WARM',
      queries: {
        create: [
          {
            platform: 'X',
            queryString: '("web3 VC" OR "crypto venture capital" OR "blockchain fund") -scam -airdrop',
            negativeKeywords: ['scam', 'airdrop', 'giveaway'],
            active: true,
          },
          {
            platform: 'REDDIT',
            queryString: '"web3 venture capital" OR "crypto VC funding"',
            subreddits: ['cryptocurrency', 'ethfinance', 'web3'],
            active: true,
          },
        ],
      },
  ...
  console.log(`  Created listening topic: ${topic2.name}`);

  const topic3 = await prisma.listeningTopic.create({
    data: {
      name: 'Portfolio Company Buzz',
      description: 'Track sentiment around portfolio companies',
      createdById: admin.id,
      active: true,
      pollingTier: 'WARM',
      queries: {
        create: [
          {
            platform: 'X',
            queryString: '"portfolio-company-alpha" OR "portfolio-company-beta"',
            active: true,
          },
        ],
      },
  ...
  console.log(`  Created listening topic: ${topic3.name}`);
